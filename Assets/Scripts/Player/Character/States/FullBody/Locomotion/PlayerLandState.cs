...
namespace AwCon
{
    ...
    public class PlayerLandState : PlayerBaseState
    {
        ...
        // Landing must preserve the crouch branch. Returning through normal Idle or MoveLoop would
        // briefly play standing locomotion before the next input frame restores crouching.
        private void ReturnToLocomotionAfterLand()
        {
            if (data.IsCrouching)
            {
                player.StateMachine.ChangeState(data.CurrentLocomotionState == LocomotionState.Idle
                    ? player.StateRegistry.GetState<PlayerCrouchIdleState>()
                    : player.StateRegistry.GetState<PlayerCrouchMoveState>());
                return;
            }
