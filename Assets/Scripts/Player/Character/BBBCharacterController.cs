...
{
    /// <summary>
    /// 整个AwCon系统的 Root 节点唯一的 Monobehaviour 驱动源 
    ...
    public class BBBCharacterController : MonoBehaviour, IDamageable, IPoolable
    {
        ...
        [Header("--- 下蹲碰撞体 ---")]
        [Range(0.45f, 0.8f)]
        [Tooltip("下蹲时 CharacterController 高度相对站立高度的比例。")]
        [SerializeField] private float crouchHeightMultiplier = 0.65f;
        ...
        private bool _isColliderCrouched;
        ...
        private void Awake()
        {
            ...
            {
                _standingColliderHeight = CharController.height;
                _standingColliderCenter = CharController.center;
            }
            ...
            FootstepController = new FootstepController(this);
        ...
        public void OnSpawned()
        {
            ...
            FootstepController?.ResetCycle();

            SyncCrouchCollider(false);
        }
        ...
        private void Update()
        {
            ...
            // 下蹲状态由 RuntimeData 统一决定，碰撞体紧跟该权威状态，
            // 不需要再为角色额外挂载下蹲适配器组件。
            SyncCrouchCollider(RuntimeData.IsCrouching);
        ...
        private void SyncCrouchCollider(bool crouch)
        {
            if (CharController == null || _standingColliderHeight <= 0f || crouch == _isColliderCrouched)
                return;

            _isColliderCrouched = crouch;
            if (crouch)
            {
                float crouchHeight = _standingColliderHeight * crouchHeightMultiplier;
                CharController.height = crouchHeight;
                CharController.center = _standingColliderCenter +
                                        Vector3.down * (_standingColliderHeight - crouchHeight) * 0.5f;
            }
            else
            {
                CharController.height = _standingColliderHeight;
                CharController.center = _standingColliderCenter;
            }
        }
