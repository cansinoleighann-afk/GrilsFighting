...
public class CharacterControl : MonoBehaviour
{
	...
	[Header("Animation")]
	public LegacyAnimancerSetSO _animancerSet;
	...
	[Header("Flying Fight")]
	public AttackSet _flyingPunchs;

	public AttackSet _flyingKicks;
	...
	private readonly Collider[] _hitBoxBuffer = new Collider[64];

	private readonly List<Collider> _hitBoxResults = new List<Collider>(64);
	...
	// Direct Animancer playback has no legacy Animator graph transition to end an
	// attack. This timestamp is advanced in Update, independent of coroutine or
	// Invoke ownership, so interrupted legacy routines cannot leave input locked.
	private float _attackEndsAt = -1f;
	...
	[HideInInspector]
	public bool _rewardInvincible;

	[HideInInspector]
	public float _rewardDamageMultiplier = 1f;
	...
	private LegacyAnimancerBridge _animancerBridge;
	...
	// Input is gathered in Update by PlayerControl/EnemyControl and applied on the physics tick.
	// Writing a dynamic Rigidbody's Transform in Update makes physics continuously correct it,
	// which presents as the visible stop-start movement reported in Level.
	private Vector3 _movementAxis;

	private Vector3 _pendingAnimationMotion;
	...
	private int _groundProbeFrame = -1;

	private bool _groundProbeHasHit;

	private RaycastHit _groundProbeHit;

	private Vector3 _groundProbeOrigin;

	private const float GroundProbeStartHeight = 0.2f;

	private const float GroundProbeDistance = 10f;

	private const float GroundSnapMaxDistance = 0.35f;

	private const float GroundRecoveryDistance = 0.5f;
	...
	public void AwakeProc()
	{
		...
		_bodyCollider = _myTransform.GetComponent<Collider>();
		_lastGroundedPosition = _myTransform.position;
		_hasLastGroundedPosition = false;
		if (_myRigidbody != null)
		{
			// Characters are moved partly by animation/Transform, so discrete collision
			// detection can tunnel through the thin floor collider.
			_myRigidbody.collisionDetectionMode = CollisionDetectionMode.ContinuousDynamic;
			_myRigidbody.interpolation = RigidbodyInterpolation.Interpolate;
		}
		_animator = GetComponentInChildren<Animator>();
		_animatorOverrideController = _animator.runtimeAnimatorController != null ? new AnimatorOverrideController(_animator.runtimeAnimatorController) : null;
		_animatorTransform = _animator.transform;
		_animancerBridge = _animator.gameObject.GetComponent<LegacyAnimancerBridge>();
		if (_animancerBridge == null) _animancerBridge = _animator.gameObject.AddComponent<LegacyAnimancerBridge>();
		_animancerBridge.Initialize(_animator, _animancerSet, _idle, _walk);
		_animationEvents = GetComponentInChildren<AnimationEvents>();
		_flicker = GetComponent<Flicker>();
		GameObject gameObject = UnityEngine.Object.Instantiate(Resources.Load("Effect/Shadow"), Vector3.zero, Quaternion.identity) as GameObject;
		_ShadowTransform = gameObject.transform;
		_ShadowTransform.parent = null;
		HitEffect = (UnityEngine.Object.Instantiate(Resources.Load("Effect/HitEffect"), Vector3.zero, Quaternion.identity) as GameObject);
		HitEffect.SetActive(value: false);
		DefendEffect = (UnityEngine.Object.Instantiate(Resources.Load("Effect/DefendEffect"), Vector3.zero, Quaternion.identity) as GameObject);
		DefendEffect.SetActive(value: false);
		DustEffectJump = (UnityEngine.Object.Instantiate(Resources.Load("Effect/DustEffectJump"), Vector3.zero, Quaternion.identity) as GameObject);
		DustEffectJump.SetActive(value: false);
		DustEffectLand = (UnityEngine.Object.Instantiate(Resources.Load("Effect/DustEffectLand"), Vector3.zero, Quaternion.identity) as GameObject);
		...
		if ((bool)GetComponent<PlayerControl>())
		{
			...
			powerUpEffectLeft = (UnityEngine.Object.Instantiate(Resources.Load("Effect/PowerUpEffect"), Vector3.zero, Quaternion.identity) as GameObject);
			...
			powerUpEffectRight = (UnityEngine.Object.Instantiate(Resources.Load("Effect/PowerUpEffect"), Vector3.zero, Quaternion.identity) as GameObject);
		...
		if (EnvironmentLayer < 0) EnvironmentLayer = LayerMask.NameToLayer("Ground");
		// This legacy project has prefabs authored on layer 8 but no serialized
		// Player/Enemy layer names. Filter hit candidates by component below,
		// rather than constructing an invalid (1 << -1) mask.
		HitLayerMask = ~0;
		AlreadyHasWeapon();
	}
	...
	public void Start()
	{
		...
		_animator = GetComponentInChildren<Animator>();
		if (_animancerBridge == null) _animancerBridge = _animator.gameObject.GetComponent<LegacyAnimancerBridge>();
		if (_animancerBridge == null) _animancerBridge = _animator.gameObject.AddComponent<LegacyAnimancerBridge>();
		_animancerBridge.Initialize(_animator, _animancerSet, _idle, _walk);
		_animatorTransform = _animator.transform;
	...
	{		
        ...
        if (!_dontRun)
		{
			ResolveAirborneState();
			if (_state == AnimStates.ATTACK && _attackEndsAt > 0f && Time.time >= _attackEndsAt)
			{
				_attackEndsAt = -1f;
				ToIDLE();
			}
	...
	{
		if (_dontRun || _myTransform == null)
		{
			return;
		}
		if (_gamePlayManager != null && _gamePlayManager._clampCharacterPositions)
		{
	...
	{
		if ((_state != AnimStates.JUMP && _state != AnimStates.AIR && _state != AnimStates.JUMPATTACK) || _myRigidbody == null)
		{
			return;
		}
		if (Time.time < _airborneSince + 0.1f || _myRigidbody.velocity.y > 0.05f)
		{
			return;
		}
		if (TryGetGround(out RaycastHit ground) && ground.distance <= 0.2f)
		{
			LandFromAir();
		}
	}
	...
	{
		if (_state != AnimStates.JUMP && _state != AnimStates.AIR && _state != AnimStates.JUMPATTACK)
		{
			return;
		}
	...
	{
		if (_dontRun || _myRigidbody == null || CantMove())
		{
			_pendingAnimationMotion = Vector3.zero;
			return;
		}

		Vector3 localVelocity = new Vector3((0f - _speed) * _powerUpSpeedMultiplyer * _movementAxis.x, 0f, _speed * _powerUpSpeedMultiplyer * _movementAxis.z);
		Vector3 displacement = _myTransform.TransformDirection(localVelocity) * Time.fixedDeltaTime + _pendingAnimationMotion;
		_pendingAnimationMotion = Vector3.zero;
		_myRigidbody.MovePosition(_myRigidbody.position + displacement);
	}

	private bool IsGroundProtectionState()
	{
		// Do not interfere with intentional airborne/knockdown motion.
		return _state != AnimStates.JUMP && _state != AnimStates.AIR && _state != AnimStates.JUMPATTACK && _state != AnimStates.KNOCKINGDOWN && _state != AnimStates.KNOCKDOWN;
	}

	private bool TryGetGround(out RaycastHit hitInfo)
	{
		if (EnvironmentLayer < 0 || _myTransform == null)
		{
			hitInfo = default(RaycastHit);
			return false;
		}
		Vector3 probeOrigin = _myTransform.position + Vector3.up * GroundProbeStartHeight;
		if (_groundProbeFrame != Time.frameCount || (_groundProbeOrigin - probeOrigin).sqrMagnitude > 0.000001f)
		{
			_groundProbeFrame = Time.frameCount;
			_groundProbeOrigin = probeOrigin;
			_groundProbeHasHit = Physics.Raycast(probeOrigin, Vector3.down, out _groundProbeHit, GroundProbeDistance, 1 << EnvironmentLayer, QueryTriggerInteraction.Ignore);
		}
		hitInfo = _groundProbeHit;
		return _groundProbeHasHit;
	}

	private void SetGroundedPosition(Vector3 position, bool resetVerticalVelocity)
	{
		_myTransform.position = position;
		if (_myRigidbody != null)
		{
			_myRigidbody.position = position;
			if (resetVerticalVelocity)
			{
				Vector3 velocity = _myRigidbody.velocity;
				_myRigidbody.velocity = new Vector3(velocity.x, 0f, velocity.z);
			}
		}
	}

	private void ProtectFromFallingThroughGround()
	{
		if (!IsGroundProtectionState() || EnvironmentLayer < 0)
		{
			return;
		}
		if (TryGetGround(out RaycastHit hitInfo))
		{
			float bottomOffset = (_bodyCollider != null) ? _bodyCollider.bounds.min.y - _myTransform.position.y : -0.05f;
			float targetY = hitInfo.point.y - bottomOffset;
			float deltaY = targetY - _myTransform.position.y;
			if (deltaY >= -GroundSnapMaxDistance && deltaY <= GroundSnapMaxDistance)
			{
				if (Mathf.Abs(deltaY) > 0.01f)
				{
					Vector3 snappedPosition = _myTransform.position;
					snappedPosition.y = targetY;
					SetGroundedPosition(snappedPosition, resetVerticalVelocity: true);
				}
				_lastGroundedPosition = _myTransform.position;
				_hasLastGroundedPosition = true;
				return;
			}
			if (deltaY > GroundRecoveryDistance && _hasLastGroundedPosition)
			{
				SetGroundedPosition(_lastGroundedPosition, resetVerticalVelocity: true);
				return;
			}
		}
		else if (_hasLastGroundedPosition && (_myTransform.position.y < _lastGroundedPosition.y - 0.12f || (_myRigidbody != null && _myRigidbody.velocity.y < -0.5f)))
		{
			SetGroundedPosition(_lastGroundedPosition, resetVerticalVelocity: true);
		}
	}
	...
	public void WalkAnimation(float value)
	{
		if (!CantMove())
		{
			_animancerBridge.SetLocomotion(value);
		}
	}

	public void Movement(Vector3 axis)
	{
		_movementAxis = CantMove() ? Vector3.zero : axis;
	}
	...
	private void AnimatorMovement()
	{
		if (_animator.transform.localPosition != Vector3.zero)
		{
			...
			_pendingAnimationMotion += a * (_direction == 1 ? _direction : -_direction);
		}
	}
	...
	private List<Collider> DrawHitBox(Vector2 CollSize)
	{
		Vector3 center = _myTransform.position + new Vector3(0f, 1f, (float)_direction * CollSize.x / 2f);
		return OverlapHitBox(center, new Vector3(_hitZdistance, CollSize.y, CollSize.x / 2f));
	}

	private List<Collider> OverlapHitBox(Vector3 center, Vector3 halfExtents)
	{
		int count = Physics.OverlapBoxNonAlloc(center, halfExtents, _hitBoxBuffer, Quaternion.identity, HitLayerMask);
		_hitBoxResults.Clear();
		for (int i = 0; i < count; i++)
		{
			_hitBoxResults.Add(_hitBoxBuffer[i]);
		}
		return _hitBoxResults;
	}

	public void Check4Hit()
	{
		List<Collider> array = DrawHitBox(_currentAttackObject.CollSize);
		bool flag = false;
		List<Collider> array2 = array;
		foreach (Collider collider in array2)
		{
			...
			BreakableObj breakable = collider.GetComponent<BreakableObj>();
			if (breakable != null)
			{
				if (!breakable._lifted)
				{
					breakable.Breake(_direction);
				}
				continue;
			}
		...
		if ((bool)_playerControl)
		{
			if (_currentAttackObject._poweredAttack || _currentAttackObject._knockDown)
			{
				MusicManage.Instance.zhen_big();
			}
			else
			{
	...
	{
		List<Collider> array = DrawHitBox(new Vector2(1f, 1.7f));
		List<Collider> array2 = array;
		foreach (Collider collider in array2)
		{
			CharacterControl target = collider.GetComponent<CharacterControl>();
			if (target != null && target != this && target._state == AnimStates.KNOCKDOWN)
			{
				return true;
			}
		}
	...
	private CharacterControl GrabFight()
	{
		...
		List<Collider> array = OverlapHitBox(center, new Vector3(0.1f, 1.7f, 0.3f));
		List<Collider> array2 = array;
		foreach (Collider collider in array2)
		{
	...
	private BreakableObj LiftObject()
	{
		...
		List<Collider> array = OverlapHitBox(center, new Vector3(1f, 1f, 0.2f));
		List<Collider> array2 = array;
		foreach (Collider collider in array2)
		{
			BreakableObj breakable = collider.GetComponent<BreakableObj>();
	...
	public void FlyingPunchCombo()
	{
		...
		_currentDamage = _punchDamage * 2f;
		if (_state == AnimStates.GRAB)
		{
			if (!_waitingGrabReaction)
			{
		...
		{
			SetAnimFloat("Speed", 0f);
			PrepeareAttack(_flyingPunchs);
		}
	}

	public void FlyingKickCombo()
	{
		if (CantAttack())
		{
			return;
	...
	public void Jump()
	{
		...
		else if (!CantJump())
		{
			...
			// The legacy controller used an Animation Event to enter the physics
			// jump. Direct Animancer clip playback has no controller transition.
			Jumping();
		}
	}
	...
	public IEnumerator MakeAttack(AttackO attackObject)
	{
		...
		// The old controller returned from attacks through graph transitions.
		// Schedule the same return from the authored clip duration so an attack
		// cannot remain locked if a legacy clip has no Animation Event.
		float attackDuration = attackObject.Attack != null && attackObject.Attack._animation != null
			? attackObject.Attack._animation.length / Mathf.Max(0.01f, attackObject.Attack._animationSpeed * _powerUpSpeedMultiplyer)
			: 0.25f;
		_attackEndsAt = Time.time + Mathf.Max(0.05f, attackDuration);
		...
		{
			// The original Animator graph supplied hit and combo-window events.
			// Direct Animancer playback owns those milestones explicitly.
			yield return new WaitForSeconds(attackDuration * 0.48f);
			if (sequence == _attackSequence && _state == AnimStates.ATTACK) Check4Hit();
			yield return new WaitForSeconds(attackDuration * 0.17f);
			if (sequence == _attackSequence && _state == AnimStates.ATTACK) AttackLock();
			yield return new WaitForSeconds(attackDuration * 0.35f);
			if (sequence == _attackSequence && _state == AnimStates.ATTACK) ToIDLE();
		}
	}
	...
	{
		if ((bool)_playerControl && _rewardInvincible)
		{
			return;
		}
		...
		if (_state == AnimStates.DEFENCE)
		{
			...
			else
			{
				if ((bool)_playerControl)
				{
					if ((attackType == AttackType.FaceAttack || attackType == AttackType.JumpAttack || attackType == AttackType.BoddyAttack || attackType == AttackType.LegAttack) && !cantEskiv && !CantEskiv())
					{
						Defence(tf: false);
						_state = AnimStates.HIT;
						...
						if (attackType == AttackType.BoddyAttack)
						{
							animationObj = _reactions._bodyEskiv[Random.Range(0, _reactions._bodyEskiv.Length)];
						}
						if (attackType == AttackType.LegAttack)
						{
							animationObj = _reactions._lowEskiv[Random.Range(0, _reactions._lowEskiv.Length)];
						}
						_gamePlayManager._makeNoise.PlaySFX(_femalePrefix + "Jump");
		...
		else if (attackType == AttackType.FaceAttack || attackType == AttackType.JumpAttack || attackType == AttackType.BoddyAttack || attackType == AttackType.LegAttack)
		{
			if ((bool)_playerControl)
			{
		...
		if (flag || flag2)
		{
			...
			if (flag)
			{
				...
				if ((bool)_enemyControl)
				{
					_gamePlayManager._hUDScript.InfoTextEffect("击倒！！", _direction);
			...
			if (flag2)
			{
				...
				if ((bool)_enemyControl)
				{
					_gamePlayManager._hUDScript.InfoTextEffect("强化！", _direction);
				}
			}
		}
		else
	...
	{
		if ((bool)_playerControl && _rewardInvincible && value < 0f)
		{
			return;
		}
	...
	public bool IsDeath()
	{
		if (_currentHealth == 0f)
		{
			...
			else if (_state == AnimStates.KNOCKINGDOWN || _state == AnimStates.KNOCKDOWN)
			{
			...
			{
				_gamePlayManager._hUDScript.InfoTextEffect("已击败", _direction);
	...
	public void SetAnimationState(string stateName, AnimationO animationObj)
	{
		...
		// The old Animator controller left these transient states through graph
		// transitions. Animancer plays the source clip directly, so finish the
		// same state transition when the non-looping clip ends.
		System.Action onEnd = (stateName == "Attack" || stateName == "Reaction") ? ToIDLE : null;
		_animancerBridge.Play(animationObj, onEnd);
	}

	private float GetClipDuration(AnimationO animation, float speedMultiplier = 1f)
	{
		if (animation == null || animation._animation == null) return 0.1f;
		return Mathf.Max(0.05f, animation._animation.length / Mathf.Max(0.01f, animation._animationSpeed * speedMultiplier));
	}

	private float GetClipDuration(AnimationClip clip, float speed = 1f)
	{
		return clip == null ? 0.1f : Mathf.Max(0.05f, clip.length / Mathf.Max(0.01f, speed));
	}

	private void PlayDirectClip(AnimationClip clip, float speed, System.Action onEnd)
	{
		if (clip == null) return;
		_animancerBridge.PlayClip(clip, speed * _powerUpSpeedMultiplyer, onEnd);
	}

	public void SetAnimTrigger(string name)
	{
		_animancerBridge.PlayState(name);
		if (name == "KnockDown") StartCoroutine(CompleteKnockDown());
	}

	public void SetAnimBool(string anim, bool tf)
	{
		if (tf) _animancerBridge.PlayState(anim);
		else if (anim == "Defence") _animancerBridge.PlayIdle();
	}

	public void SetAnimFloat(string anim, float value)
	{
		if (anim == "Speed") _animancerBridge.SetLocomotion(value);
	}
	...
	public void StandUp()
	{
		if (_state != AnimStates.KNOCKINGDOWN && _state != AnimStates.DEATH && _state != AnimStates.JUMP && _state != AnimStates.JUMPATTACK && _state != AnimStates.AIR)
		{
			...
			StartCoroutine(CompleteStandUp());
		}
	}

	private IEnumerator CompleteStandUp()
	{
		float duration = _animancerSet != null && _animancerSet.standUp != null ? _animancerSet.standUp.length : 0.5f;
		yield return new WaitForSeconds(Mathf.Max(0.05f, duration));
		if (_state == AnimStates.STANDINGUP) ToIDLE();
	}

	private IEnumerator CompleteKnockDown()
	{
		float duration = _animancerSet != null && _animancerSet.knockdown != null ? _animancerSet.knockdown.length : 0.5f;
		yield return new WaitForSeconds(Mathf.Max(0.05f, duration));
		if (_state == AnimStates.KNOCKINGDOWN) KnockDown();
	}
	...
	private void OnCollisionExit(Collision collision)
	{
		if (!_dontRun && _state != AnimStates.DEATH && collision.gameObject.layer == EnvironmentLayer)
		{
			...
			{
				// A Transform-driven step can briefly emit CollisionExit before the
				// physics engine catches up. Keep the grounded state so the fallback
				// detector can restore the last valid floor position.
				if (_hasLastGroundedPosition && _myTransform.position.y <= _lastGroundedPosition.y + 0.2f)
				{
					return;
				}
	...
	public void CheckWeaponInRange()
	{
		...
		{
			float pickUpRange = availableWeapon._pickUpRange;
			float distanceSquared = (position - availableWeapon._myTransform.position).sqrMagnitude;
			if (distanceSquared < pickUpRange * pickUpRange && !availableWeapon._picked)
			{
	...
	private bool CanPickWeapon()
	{
		...
		float num = (_myTransform.position - _weaponsInRange[0]._myTransform.position).sqrMagnitude;
		for (int i = 0; i < _weaponsInRange.Count; i++)
		{
			if ((_myTransform.position - _weaponsInRange[i]._myTransform.position).sqrMagnitude < num && !weapons._picked)
			{
	...
	private void ThrowWeapon(Weapons weapon)
	{
		...
		StartCoroutine(CompleteWeaponThrow(weapon, animationO));
	}

	private IEnumerator CompleteWeaponThrow(Weapons weapon, AnimationO animation)
	{
		float duration = GetClipDuration(animation, _powerUpSpeedMultiplyer);
		yield return new WaitForSeconds(duration * 0.55f);
		if (_throwedWeapon == weapon) WeaponThrowed();
		yield return new WaitForSeconds(duration * 0.45f);
		if (_state == AnimStates.ATTACK) ToIDLE();
	}

	public void WeaponThrowed()
	{
		if (_throwedWeapon == null) return;
	...
	private void PickWeapon(Weapons weapon, float ind)
	{
		...
		StartCoroutine(CompleteWeaponPickup(weapon));
	}

	private IEnumerator CompleteWeaponPickup(Weapons weapon)
	{
		float duration = _animancerSet != null && _animancerSet.pickup != null ? _animancerSet.pickup.length : 0.5f;
		yield return new WaitForSeconds(duration * 0.5f);
		if (_state == AnimStates.PICK && _pickedWeapon == weapon) WeaponPicked();
		yield return new WaitForSeconds(duration * 0.5f);
		if (_state == AnimStates.PICK) ToIDLE();
	}
	...
	public void GrantWeaponToRightHand(int weaponId)
	{
		if (weaponId <= 0 || weaponId >= Config.WeaponNames.Length || _holdedWeaopons.Count > 0)
		{
			return;
		}

		_rightHandWeapon = weaponId;
		AlreadyHasWeapon();
	}
	...
	private void Lift(BreakableObj breakableObj)
	{
		...
		StartCoroutine(CompleteLift(breakableObj));
	}

	private IEnumerator CompleteLift(BreakableObj lifted)
	{
		float duration = GetClipDuration(lifted != null ? lifted._liftAnimation : null);
		yield return new WaitForSeconds(duration * 0.45f);
		if (_lifting && _liftedObj == lifted) Lifting();
		yield return new WaitForSeconds(duration * 0.55f);
		if (_lifting && _liftedObj == lifted) Lifted();
	}
	...
	public void Lifted()
	{
		_attackLock = false;
		_gamePlayManager._makeNoise.PlaySFX(_femalePrefix + "Land");
		_animancerBridge.PlayClip(_liftedObj._liftOwerWriteAnimation._animation);
	}
	...
	private void LiftThrow()
	{
		...
		StartCoroutine(CompleteLiftThrow(_liftedObj));
	}

	private IEnumerator CompleteLiftThrow(BreakableObj lifted)
	{
		float duration = GetClipDuration(lifted != null ? lifted._throwAttack.Attack : null, _powerUpSpeedMultiplyer);
		yield return new WaitForSeconds(duration * 0.55f);
		if (_lifting && _liftedObj == lifted) LiftThrowed();
		yield return new WaitForSeconds(duration * 0.45f);
		if (_state == AnimStates.ATTACK) ToIDLE();
	}
	...
	private void GrabEmpty()
	{
		if (_state == AnimStates.IDLE)
		{
			_attackLock = true;
			_state = AnimStates.ATTACK;
			PlayDirectClip(_grabEmpty._animation, _grabEmpty._animationSpeed, null);
			StartCoroutine(CompleteEmptyGrab());
		}
	}

	private IEnumerator CompleteEmptyGrab()
	{
		yield return new WaitForSeconds(GetClipDuration(_grabEmpty));
		if (_state == AnimStates.ATTACK) ToIDLE();
	}
	...
	public void Grabbing(CharacterControl enemyCharacterControl)
	{
		...
		if (_enemyCharacterControl._state == AnimStates.DEFENCE)
		{
			CancelInvoke("Escape");
			Invoke("Escape", 0.25f);
			PlayDirectClip(_currentGrab._grabbCombos[0].Attack, _currentGrab._grabbCombos[0]._animSpeed, null);
			return;
		}
		_grabbed = true;
	...
	public void GrabbingAttack()
	{
		if (!_waitingGrabReaction)
		{
			...
			GrabAnimations combo = _currentGrab._grabbCombos[_currentGrab._comboInd];
			PlayDirectClip(combo.Attack, combo._animSpeed, null);
			_enemyCharacterControl.PlayDirectClip(combo.Reaction, combo._animSpeed, null);
			StartCoroutine(ResolveGrabCombo(combo));
		}
	}

	private IEnumerator ResolveGrabCombo(GrabAnimations combo)
	{
		float duration = GetClipDuration(combo != null ? combo.Attack : null, combo != null ? combo._animSpeed : 1f);
		yield return new WaitForSeconds(duration * 0.48f);
		if (_grabbed && _waitingGrabReaction) GrabHit();
		yield return new WaitForSeconds(duration * 0.52f);
		if (_grabbed && _waitingGrabReaction) GrabReaction();
	}
	...
	public void GrabHit()
	{
		...
		_enemyCharacterControl.SetHealth(0f - _grabDamage * _rewardDamageMultiplier);
	...
	{
		PlayDirectClip(_currentGrab._escape.Attack, _currentGrab._escape._animSpeed, ToIDLE);
	}

	private void ReleaseEnemy()
	{
		_enemyCharacterControl.SetAnimBool("Defence", tf: false);
		_enemyCharacterControl.PlayDirectClip(_currentGrab._escape.Reaction, _currentGrab._escape._animSpeed, _enemyCharacterControl.ToIDLE);
	}
