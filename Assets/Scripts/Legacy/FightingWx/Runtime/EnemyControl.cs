...
public class EnemyControl : MonoBehaviour
{
	...
	public void UpdateProc()
	{
		...
		{
			float distanceSquared = (_characterControl._myTransform.position - _playerCharacterControl._myTransform.position).sqrMagnitude;
			if (!_iSaw && distanceSquared < _seeDistance * _seeDistance)
			{
				_iSaw = true;
			}
			if (_iSaw && !Combat())
	...
	private bool Combat()
	{
		float distanceSquared = (destinationPos - _characterControl._myTransform.position).sqrMagnitude;
		if (distanceSquared > 0.25f && distanceSquared <= 12.25f && !CantCombo())
		{
			...
			if (_characterControl._lifting && distanceSquared <= 4f)
			{
				_characterControl.PunchCombo();
				return true;
			}
			if (UnityEngine.Random.Range(0, 100) <= _jumpAttackFreq && _gonnaPickWeapon == null && distanceSquared <= 2.25f)
			{
		...
		if (distanceSquared <= 0.25f || _characterControl._state == AnimStates.GRAB)
		{
	...
	private void Movement()
	{
		...
		if (flag)
		{
			float playerDistanceSquared = (_characterControl._myTransform.position - _playerCharacterControl._myTransform.position).sqrMagnitude;
			float weaponDistanceSquared = (_characterControl._myTransform.position - _gonnaPickWeapon._myTransform.position).sqrMagnitude;
			if (playerDistanceSquared < weaponDistanceSquared)
			{
	...
	private void WeaponChecker()
	{
		if (_canPickWeapon)
		{
			...
			if (_characterControl._holdedWeaopons.Count < 2)
			{
				...
				{
					if ((_characterControl._myTransform.position - availableWeapon._myTransform.position).sqrMagnitude < playerDistanceSquared && !availableWeapon._picked && !availableWeapon._enemyWantsToPick && !availableWeapon._enemyCantPick)
					{
			...
			if (_canPickableWeapons.Count > 0)
			{
				...
				float closestDistanceSquared = (_characterControl._myTransform.position - weapons._myTransform.position).sqrMagnitude;
				foreach (Weapons canPickableWeapon in _canPickableWeapons)
				{
					float candidateDistanceSquared = (_characterControl._myTransform.position - canPickableWeapon._myTransform.position).sqrMagnitude;
