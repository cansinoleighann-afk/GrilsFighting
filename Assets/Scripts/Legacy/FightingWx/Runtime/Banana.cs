...
public class Banana : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (_pickable)
		{
			float pickUpRange = _pickUpRange;
			float distanceSquared = (_gamePlayManager._playerControl._characterControl._myTransform.position - _myTransform.position).sqrMagnitude;
			if (distanceSquared <= pickUpRange * pickUpRange)
			{
			...
			foreach (EnemyControl activeEnemy in _gamePlayManager._activeEnemies)
			{
				...
				{
					float enemyDistanceSquared = (activeEnemy._characterControl._myTransform.position - _myTransform.position).sqrMagnitude;
					if (enemyDistanceSquared <= pickUpRange * pickUpRange)
					{
						_pickUpEffect.SetActive(value: true);
						_pickUpEffect.transform.parent = null;
