...
public class MakeNoise : MonoBehaviour
{
	...
	{
		if (DataTools.getInt(Config.PP_Sound) == 0)
		{
			return;
		}
		...
		foreach (AudioClips audioClips in audioList)
		{
			...
			audioSource.volume = audioClips.volume * sfxVolume * (float)DataTools.getInt(Config.PP_Sound);
	...
	public void ChangeVolumeImmediate()
	{
		...
		{
			audioSource.volume *= DataTools.getInt(Config.PP_Sound);
		}
	}
