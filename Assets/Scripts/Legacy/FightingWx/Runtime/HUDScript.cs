...
public class HUDScript : MonoBehaviour
{
	...
	private float[] _infoTextHideAt;

	private const float EffectTextDuration = 2f;
	...
	private void Awake()
	{
		...
		_hitTextHideAt = new float[_hitText.Length];
		...
		_infoTextHideAt = new float[_InfoText.Length];
	...
	public void HitTextEffect(int direction)
	{
		...
		_hitText[effectInd].text = _hitValue + " 连击！";
		_hitTextHideAt[effectInd] = Time.unscaledTime + EffectTextDuration;
	...
	public void InfoTextEffect(string text, int direction)
	{
		...
		_infoTextHideAt[effectInd] = Time.unscaledTime + EffectTextDuration;
	...
	public void ChangeLevelName(int level)
	{
		...
		{
			text.text = "第 " + level + " 关完成";
		}
		int intData = DataTools.getInt(Config.PP_Diffuculty);
	...
	private void Update()
	{
		...
		for (int i = 0; i < _hitText.Length; i++)
		{
			if (_hitText[i] != null && !string.IsNullOrEmpty(_hitText[i].text) && now >= _hitTextHideAt[i])
			{
				_hitText[i].text = string.Empty;
			}
		}
		...
		{
			if (_InfoText[i] != null && !string.IsNullOrEmpty(_InfoText[i].text) && now >= _infoTextHideAt[i])
			{
				_InfoText[i].text = string.Empty;
			}
		}
