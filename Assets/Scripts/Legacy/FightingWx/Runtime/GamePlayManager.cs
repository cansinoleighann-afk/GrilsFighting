...
public class GamePlayManager : MonoBehaviour
{
	...
	private static readonly Dictionary<int, int> LevelAttempts = new Dictionary<int, int>();
	...
	private void Awake()
	{
		...
		if (GetComponent<RewardBuffController>() == null)
		{
			base.gameObject.AddComponent<RewardBuffController>();
		}
		...
		_stageAttempt = GetNextLevelAttempt(_currentLevel);
		...
		GameObject playerspawnpos = GameObject.Find("PlayerSpawnPos");

        GameObject gameObject = UnityEngine.Object.Instantiate(_playerPrefab, playerspawnpos.transform.position, Quaternion.identity); 
		...
		GiveSuperStartWeaponIfNeeded();
		//_playerControl._characterControl._myTransform.position = playerspawnpos.transform.position;
		...
		float num = Config.DiffucultyMultiplayers[DataTools.getInt(Config.PP_Diffuculty)];
		...
		_totalOranges = DataTools.getInt(Config.PP_Orange_Count);
	...
	private void CalculateLeftRightBounds()
	{
		...
		Vector3 vector = _frustumPlanes[0].ClosestPointOnPlane(_playerControl._characterControl._myTransform.position);
		Vector3 vector2 = _frustumPlanes[1].ClosestPointOnPlane(_playerControl._characterControl._myTransform.position);
	...
	{
		if (AdCon.self.ovData != null)
		{
			AdCon.self.ovData.show_event("over1");
		}
	}

	private static int GetNextLevelAttempt(int level)
	{
		int attempt = GetUpcomingLevelAttempt(level);
		LevelAttempts[level] = attempt;
		return attempt;
	}

	public static int GetUpcomingLevelAttempt(int level)
	{
		int attempt;
		LevelAttempts.TryGetValue(level, out attempt);
		return attempt + 1;
	}
	...
	public void PlayerDie()
	{
		...
		eventData.Add("deadth", "玩家死亡");
		eventData.Add("level", _currentLevel.ToString());
		WebSdk.self.send_AddEvent("PlayerDead", eventData);
		_gameOver = true;
		_camFollow.Reset();
	...
	{
		ReportGameEnd();
		DataTools.save(Config.PP_Orange_Count, _totalOranges);
	...
	private void LevelCompLeted()
	{
		...
		ReportGameEnd();
		Dictionary<string, string> eventData = new Dictionary<string, string>();
		eventData.Add("state", "关卡完成");
		eventData.Add("level", _currentLevel.ToString());
		eventData.Add("attempt", _stageAttempt.ToString());
		WebSdk.self.send_AddEvent("Over", eventData);
		DataTools.save(Config.PP_Orange_Count, _totalOranges);
	...
	private void GiveSuperStartWeaponIfNeeded()
	{
		if (GlobalVariables._superStartWeapon <= 0)
		{
			return;
		}

		_playerControl._characterControl.GrantWeaponToRightHand(GlobalVariables._superStartWeapon);
