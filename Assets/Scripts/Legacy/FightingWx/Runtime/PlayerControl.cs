...
public class PlayerControl : MonoBehaviour
{
	...
	{
		_healthU = _health + (float)DataTools.getInt(_id.ToString() + Config.PP_CharacterHealthUpgradeCount) * 0.5f;
		_weaponTecniqueU = _weaponTecnique + (float)DataTools.getInt(_id.ToString() + Config.PP_CharacterWeaponTecniqueUpgradeCount) * 0.2f;
		_punchPowerU = _punchPower + (float)DataTools.getInt(_id.ToString() + Config.PP_CharacterPunchPowerUpgradeCount) * 0.2f;
		_kickPowerU = _kickPower + (float)DataTools.getInt(_id.ToString() + Config.PP_CharacterKickPowerUpgradeCount) * 0.2f;
	}
	...
	private void Combat()
	{
		...
		if (CF2Input.GetKeyDown(KeyCode.LeftArrow) || CF2Input.GetKeyDown(KeyCode.RightArrow))
		{
			_flyin = true;
			CancelInvoke("Flying");
			Invoke("Flying", 0.15f);
